...
class Dset(object):
    def __init__(self, inputs, labels, rews, next_obs, dones, randomize):
        self.inputs = inputs
        self.labels = labels
        self.rews = rews
    ...
    def get_next_batch(self, batch_size):
        ...
        rews = self.rews[self.pointer:end]
        next_obs = self.next_obs[self.pointer:end]
...
class MujocoDset(object):
    def __init__(self, expert_path, train_fraction=0.7, traj_limitation=-1, randomize=True):
        ...
        rews = traj_data['rews'][:traj_limitation]
        next_obs = traj_data['next_obs'][:traj_limitation]
        dones = traj_data['dones'][:traj_limitation]
        ...
        if len(obs.shape[2:]) != 0:
            ...
            self.next_obs = np.reshape(next_obs, [-1, np.prod(obs.shape[2:])])
        else:
    ...
    def get_all_data(self):
        return self.obs, self.acs, self.rews, self.next_obs, self.dones
